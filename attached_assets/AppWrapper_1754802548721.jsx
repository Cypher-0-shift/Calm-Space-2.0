...
const AppWrapper = ({ children }) => {
  ...
  return (
    <div
      className={`min-h-screen flex flex-col bg-gradient-to-br transition-all duration-700`}
    >
      {/* Top Navbar - Desktop */}
      <nav className="hidden md:flex fixed top-0 left-0 right-0 z-50 justify-center items-center bg-white/30 backdrop-blur-lg shadow-md px-6 py-3 space-x-3 sm:space-x-6">
      ...
      <nav className="md:hidden fixed bottom-0 left-0 right-0 z-50 bg-white/80 dark:bg-black/60 backdrop-blur-md shadow-inner flex justify-around py-2 px-4 border-t border-gray-200 dark:border-gray-700">
        {navItems.map(({ name, path, icon, id }) => (
          <Link
            key={id}
            to={path}
      ...
      {/* Main Content */}
      <main className="relative z-[1] flex-grow pt-[6rem] pb-[5rem] px-4 sm:px-8 w-full">
        {children}
        <DevResetButton />
      </main>
    </div>
